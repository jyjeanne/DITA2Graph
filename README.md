...
| Hybrid graph+RAG architecture (§13.1) | Mostly shipped: body-text extraction, `rag/chunks.jsonl` + `rag/metadata.json` (same single pass as `okf/`), `search_content` (graph-narrowed content search, verified to actually narrow), and `analyze_impact` (reverse, transitive graph traversal, now with a text excerpt per affected concept). Still design-only: ranked/semantic content matching and node-level embeddings |
...
# Inspect the RAG content index written alongside it (§13.1, not yet
# consumed by anything -- no search/embeddings/MCP tool built on it yet)
